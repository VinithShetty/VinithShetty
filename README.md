<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:8E2DE2,100:4A00E0&height=200&section=header&text=Vinith%20Shetty&fontSize=52&fontColor=ffffff&fontAlignY=40&animation=fadeIn&v=2" width="100%" />

<a href="https://readme-typing-svg.demolab.com">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&pause=1000&color=A855F7&center=true&vCenter=true&width=680&height=55&lines=AI+Engineer+%40+iSN Global Solutions;LLM+%2B+RAG+%2B+AI+Agent+Architect;FastAPI+%7C+Python+%7C+Vector+Databases;Building+Intelligent+Systems+That+Scale" alt="Typing SVG" />
</a>

<br/>

![Degree](https://img.shields.io/badge/B.E.-Artificial%20Intelligence%20%26%20Data%20Science-7C3AED?style=for-the-badge)
![University](https://img.shields.io/badge/Mumbai%20University-Class%20of%202026-4F46E5?style=for-the-badge)
![CGPA](https://img.shields.io/badge/CGPA-7.88%2F10-8B5CF6?style=for-the-badge)
![Location](https://img.shields.io/badge/Mumbai%2C%20India-A855F7?style=for-the-badge&logo=googlemaps&logoColor=white)

<br/>

<a href="https://vinithportfolio.vinithshetty96.workers.dev">
  <img src="https://img.shields.io/badge/Portfolio-6D28D9?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
</a>
<a href="https://www.linkedin.com/in/vinith-shetty879">
  <img src="https://img.shields.io/badge/LinkedIn-4F46E5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="mailto:vinithshetty96@gmail.com">
  <img src="https://img.shields.io/badge/Email-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
<a href="https://github.com/VinithShetty">
  <img src="https://img.shields.io/badge/GitHub-4A00E0?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=VinithShetty&label=Profile%20Views&color=8B5CF6&style=for-the-badge" alt="Profile Views" />
<img src="https://img.shields.io/github/followers/VinithShetty?label=Followers&style=for-the-badge&color=7C3AED&logo=github&logoColor=white" alt="Followers" />
<img src="https://img.shields.io/github/stars/VinithShetty?label=Stars&style=for-the-badge&color=A855F7&logo=github&logoColor=white" alt="Stars" />

</div>

---

## ▍About Me

I'm an **AI Engineer** who builds production-grade intelligent systems — from **LLM and RAG architectures** to full-stack applications and automation pipelines that solve real business problems. My work sits at the intersection of **software engineering rigor** and **applied AI**: designing scalable **FastAPI** backends, engineering retrieval systems over **vector databases**, and shipping containerized services with **Docker, Kubernetes, and CI/CD**.

I care about the entire lifecycle — clean architecture, measurable impact, and systems that hold up in production, not just in a notebook. I approach every project with a **product engineering mindset**: understanding the problem, designing for scale, and shipping something people actually use. Currently completing my **B.E. in AI & Data Science** while building tools that turn manual workflows into intelligent automation.

- Engineering **production AI** — LLMs, RAG, AI agents, and prompt-optimized inference pipelines
- Building **full-stack** — React / Next.js frontends backed by Python & FastAPI microservices
- Shipping with **DevOps discipline** — Docker, Kubernetes, GitHub Actions, and observability in mind

<br/>

<div align="center">

**◆ Open To ◆**

![Full-Time](https://img.shields.io/badge/Full--Time-AI%20%2F%20ML%20Engineer-6D28D9?style=flat-square)
![Backend](https://img.shields.io/badge/Backend%20%26%20Platform-Engineering-7C3AED?style=flat-square)
![GenAI](https://img.shields.io/badge/Applied%20AI%20%26%20GenAI-Roles-8B5CF6?style=flat-square)
![OSS](https://img.shields.io/badge/Open%20Source-Collaboration-A855F7?style=flat-square)

</div>

---

## ▍Tech Stack

<div align="center">

**Languages**

<img src="https://skillicons.dev/icons?i=py,c,js,html,css&theme=dark" alt="Languages" />
&nbsp;
<img src="https://img.shields.io/badge/SQL-4F46E5?style=flat-square&logo=postgresql&logoColor=white" alt="SQL" />

<br/>

**Frontend**

<img src="https://skillicons.dev/icons?i=react,nextjs,html,css,js&theme=dark" alt="Frontend" />

<br/>

**Backend &amp; Databases**

<img src="https://skillicons.dev/icons?i=fastapi,flask,nodejs,mongodb,postgres,supabase&theme=dark" alt="Backend and Databases" />
<br/>
<img src="https://img.shields.io/badge/Pinecone-6D28D9?style=flat-square&logo=pinecone&logoColor=white" alt="Pinecone" />
<img src="https://img.shields.io/badge/Qdrant-7C3AED?style=flat-square" alt="Qdrant" />
<img src="https://img.shields.io/badge/REST%20APIs-8B5CF6?style=flat-square" alt="REST APIs" />

<br/>

**AI / ML &amp; Data**

![LLMs](https://img.shields.io/badge/LLMs-4A00E0?style=flat-square)
![RAG](https://img.shields.io/badge/RAG-6D28D9?style=flat-square)
![Vector DB](https://img.shields.io/badge/Vector%20Databases-7C3AED?style=flat-square)
![Groq](https://img.shields.io/badge/Groq-4F46E5?style=flat-square)
![NumPy](https://img.shields.io/badge/NumPy-8B5CF6?style=flat-square&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-6D28D9?style=flat-square&logo=pandas&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-7C3AED?style=flat-square&logo=scikitlearn&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-A855F7?style=flat-square)

<br/>

**Cloud, DevOps &amp; Tooling**

<img src="https://skillicons.dev/icons?i=docker,kubernetes,git,github,githubactions,vscode&theme=dark" alt="DevOps and Tooling" />
<br/>
<img src="https://img.shields.io/badge/Cursor-6D28D9?style=flat-square" alt="Cursor" />
<img src="https://img.shields.io/badge/Claude%20Code-7C3AED?style=flat-square&logo=anthropic&logoColor=white" alt="Claude Code" />
<img src="https://img.shields.io/badge/n8n-A855F7?style=flat-square&logo=n8n&logoColor=white" alt="n8n" />

</div>

---

## ▍AI / ML Expertise

<div align="center">

| Domain | Proficiency | Details |
| :--- | :---: | :--- |
| **Large Language Models** | `████████░░` Advanced | Applied GPT-4 &amp; Claude across code review and conversational systems |
| **Retrieval-Augmented Generation** | `████████░░` Advanced | Repository-aware &amp; document-grounded RAG over Qdrant and Pinecone |
| **AI Agents &amp; Orchestration** | `███████░░░` Proficient | Workflow orchestration and LLM-assisted decision engines |
| **Prompt Engineering** | `████████░░` Advanced | Optimized prompts for accuracy, latency, and contextual relevance |
| **Vector Databases** | `███████░░░` Proficient | Embedding pipelines with Pinecone &amp; Qdrant semantic search |
| **AI Automation &amp; Pipelines** | `████████░░` Advanced | REST-based automation with n8n, SERPAPI, and Python at scale |
| **Production AI Systems** | `███████░░░` Proficient | FastAPI microservices, Docker, Kubernetes, and CI/CD delivery |

</div>

---

## ▍Featured Projects

<details open>
<summary><b>🟣 AI-Powered Code Review Assistant</b></summary>
<br/>

Production-grade platform that automates pull-request analysis using LLMs, delivering repository-aware, security-focused reviews in real time.

| | |
| :--- | :--- |
| **Stack** | FastAPI · GPT-4 · Claude · Qdrant · Docker · Kubernetes · GitHub Actions |
| **Scale** | Repo-aware RAG pipeline with embedding-based semantic search across full codebases |
| **Performance** | Real-time reviews auto-triggered via GitHub OAuth, webhooks, and REST APIs |
| **Security** | OWASP-based detection of vulnerabilities, exposed secrets, and code smells |
| **Impact** | Cut manual review effort while improving review consistency and developer productivity |
| **Repository** | [View on GitHub »](https://github.com/VinithShetty/AI_Code_review_Assistant.git) |

Architected an end-to-end AI review system: a scalable FastAPI microservices backend integrates with GitHub to trigger analysis on PR events, a Retrieval-Augmented Generation pipeline over Qdrant supplies contextual repository awareness, and an analytics dashboard surfaces review statistics, developer activity, and code-quality trends. The full application is containerized with Docker and Kubernetes and deployed through automated CI/CD with GitHub Actions.

</details>

<details>
<summary><b>🟣 VCET Helpdesk RAG Chatbot</b></summary>
<br/>

Production-ready RAG chatbot that answers institutional queries accurately by combining semantic search with large language models.

| | |
| :--- | :--- |
| **Stack** | FastAPI · Pinecone · Groq LLM · Python |
| **Scale** | 50+ institutional documents indexed into a Pinecone vector database |
| **Performance** | Low-latency, high-quality responses via Groq inference APIs |
| **Security** | Institution-scoped retrieval that minimizes irrelevant and off-topic answers |
| **Impact** | Instant, context-aware information access for students, faculty, and visitors |
| **Repository** | [Live Demo »](https://vinith123-vcet-chatbot.hf.space/) |

Built an embedding pipeline that indexes institutional documents for efficient semantic retrieval, integrated Groq for fast conversational inference, and optimized prompt engineering for answer accuracy. Delivered scalable FastAPI endpoints behind an interactive web interface so the entire campus community can self-serve institutional information.

</details>

<details>
<summary><b>🟣 SwasthSahayak — AI Healthcare Voice Assistant</b></summary>
<br/>

Multilingual, voice-first healthcare assistant built at HackCelestial to improve rural healthcare accessibility through conversational AI.

| | |
| :--- | :--- |
| **Stack** | Conversational AI · Voice-First Interaction · Multilingual NLP · Python |
| **Scale** | Designed for rural, low-connectivity healthcare access |
| **Performance** | Real-time, natural voice interaction for hands-free use |
| **Security** | Privacy-conscious, voice-driven query handling |
| **Impact** | Bridges the rural healthcare accessibility gap with an intuitive voice interface |

Developed during a competitive hackathon, SwasthSahayak applies conversational AI principles and voice-first design to make health information reachable for users who face literacy or connectivity barriers — turning a complex healthcare access problem into a simple spoken conversation.

</details>

---

## ▍Experience

**AI Engineer Intern** · *ASB Engineering Services* &nbsp;·&nbsp; `Nov 2025 – Jan 2026` &nbsp;·&nbsp; Mumbai, India

Designed and deployed a production-ready AI automation platform that streamlined lead discovery, research, verification, and qualification — reducing manual effort by **10+ hours per week**.

- Engineered AI-powered workflow orchestration integrating **SERPAPI, MailboxLayer, Google Sheets API**, and Python automation into scalable business-intelligence pipelines
- Built an **LLM-assisted lead scoring engine** combining confidence scores, verification metrics, and business rules to automate prospect qualification
- Developed REST-based automation pipelines processing and validating **100+ business leads** with zero manual intervention
- Optimized API integrations for real-time data collection, validation, and synchronization across multiple external services
- Partnered with stakeholders to convert manual research into scalable AI-assisted systems, accelerating business development

<div align="center">

![FastAPI](https://img.shields.io/badge/FastAPI-6D28D9?style=flat-square&logo=fastapi&logoColor=white)
![Python](https://img.shields.io/badge/Python-4F46E5?style=flat-square&logo=python&logoColor=white)
![LLMs](https://img.shields.io/badge/LLMs-7C3AED?style=flat-square)
![REST APIs](https://img.shields.io/badge/REST%20APIs-8B5CF6?style=flat-square)
![Automation](https://img.shields.io/badge/AI%20Automation-A855F7?style=flat-square)
![n8n](https://img.shields.io/badge/n8n-6D28D9?style=flat-square&logo=n8n&logoColor=white)

</div>

---

## ▍Achievements

<div align="center">

| Recognition | Details |
| :--- | :--- |
| **HackCelestial** | Built *SwasthSahayak*, an AI-powered multilingual voice healthcare assistant for rural accessibility |
| **Code-O-Fiesta** | Developed a full-stack Entrepreneurs Community Platform for networking and startup collaboration |
| **Microsoft Learn Student Club** | Outreach Associate — coordinated technical workshops, coding events, and AI community initiatives |
| **National Students Data Corps (NSDC)** | Deputy Social Media Head — led digital outreach, content strategy, and event campaigns |

</div>

---

## ▍Certifications

<div align="center">

**Amazon Web Services**

[![AWS](https://img.shields.io/badge/AWS-Certification-4F46E5?style=for-the-badge&logo=amazonwebservices&logoColor=white)](#)

**IBM**

[![Oracle](https://img.shields.io/badge/IBM-Certification-6D28D9?style=for-the-badge&logo=oracle&logoColor=white)](#)

**Infosys Springboard**

[![Infosys](https://img.shields.io/badge/Infosys-Certification-7C3AED?style=for-the-badge&logo=googlescholar&logoColor=white)](#)

**HP**

[![HP](https://img.shields.io/badge/HP-Certification-8B5CF6?style=for-the-badge&logo=hp&logoColor=white)](#)

</div>

---

## ▍Coding Profiles

<div align="center">

[![LeetCode](https://img.shields.io/badge/LeetCode-Profile-4A00E0?style=for-the-badge&logo=leetcode&logoColor=white)](https://leetcode.com/u/VinithShetty)
[![GeeksforGeeks](https://img.shields.io/badge/GeeksforGeeks-Profile-6D28D9?style=for-the-badge&logo=geeksforgeeks&logoColor=white)](https://auth.geeksforgeeks.org/user/VinithShetty)
[![HackerRank](https://img.shields.io/badge/HackerRank-Profile-7C3AED?style=for-the-badge&logo=hackerrank&logoColor=white)](https://www.hackerrank.com/VinithShetty)
[![CodeChef](https://img.shields.io/badge/CodeChef-Profile-8B5CF6?style=for-the-badge&logo=codechef&logoColor=white)](https://www.codechef.com/users/VinithShetty)

</div>

---

## ▍GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=VinithShetty&show_icons=true&hide_border=true&count_private=true&title_color=A855F7&icon_color=8B5CF6&text_color=C9D1D9&bg_color=0D1117&v=2" alt="GitHub Stats" />
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=VinithShetty&layout=compact&hide_border=true&title_color=A855F7&text_color=C9D1D9&bg_color=0D1117&langs_count=8&v=2" alt="Top Languages" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=VinithShetty&hide_border=true&background=0D1117&stroke=8B5CF6&ring=A855F7&fire=A855F7&currStreakLabel=A855F7&sideLabels=C9D1D9&currStreakNum=ffffff&sideNums=ffffff&dates=8B92A6&v=2" alt="GitHub Streak" />

</div>

---

## ▍GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=VinithShetty&theme=algolia&no-frame=true&no-bg=true&margin-w=4&column=7&v=2" alt="GitHub Trophies" />

</div>

---

## ▍Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=VinithShetty&bg_color=0D1117&color=A855F7&line=8B5CF6&point=ffffff&area=true&area_color=4A00E0&hide_border=true&custom_title=Contribution%20Graph&v=2" alt="Contribution Activity Graph" width="100%" />

</div>

---

## ▍Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/VinithShetty/VinithShetty/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/VinithShetty/VinithShetty/output/github-contribution-grid-snake.svg" />
  <img alt="Contribution Snake Animation" src="https://raw.githubusercontent.com/VinithShetty/VinithShetty/output/github-contribution-grid-snake-dark.svg" />
</picture>

</div>

---

## ▍Current Focus

```yaml
Vinith Shetty:
  role: "AI Engineer"
  focus: "Production-grade LLM & RAG systems"

  learning:
    - "Advanced AI agent orchestration & multi-step reasoning"
    - "LLM evaluation, guardrails & observability"
    - "Scalable distributed systems on Kubernetes"

  building:
    - "Repository-aware AI code review tooling"
    - "Retrieval systems over vector databases"
    - "Automation pipelines that replace manual workflows"

  exploring:
    - "Model context protocols & tool-using agents"
    - "Cost- and latency-optimized inference"

  open_to:
    - "Full-time AI / ML Engineer roles"
    - "Backend & platform engineering"
    - "Open-source collaboration"
```

---

## ▍Connect

<div align="center">

<a href="mailto:vinithshetty96@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
</a>
<a href="https://www.linkedin.com/in/vinith-shetty879">
  <img src="https://img.shields.io/badge/LinkedIn-4F46E5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="https://github.com/VinithShetty">
  <img src="https://img.shields.io/badge/GitHub-4A00E0?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>
<a href="https://vinithportfolio.vinithshetty96.workers.dev">
  <img src="https://img.shields.io/badge/Portfolio-6D28D9?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
</a>

</div>

---

<div align="center">

*"Engineering intelligent systems that turn real-world problems into production-ready solutions."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4A00E0,100:8E2DE2&height=120&section=footer&v=2" width="100%" />

</div>
